   ...
   var preparedResultBoard = (careerInterest) => {
       // change the header
           if (careerInterest.length >= 1) {
                ...
                <div class="footer">
                    <a href="field-of-study.html" class="btn btn-large green white-text">Proceed<i class="fa fa-left right"></i></a>
                </div>`;
           }else{
                _$('.question-box').innerHTML = 
                `<p class="center green-text " style="font-size:2.68rem; font-weight: bold">Congratulation <i class="fa fa-check"></i> </p>
...
// DATA STRUCTURE CONTROLLER
   var dataStructure = function (action, id, data) {
       if (action === 'add') {
        ...
        //    var index = selectedActivity.indexOf(data);
        //     if (index > -1) {
...
// DISPLAY THE PREVIOUS CAREER RESULT ON THE previous-result.html ONCE THE PAGE LOADS
    ...
    if(_$('#previous-test'))
    _$('#previous-test').addEventListener('DOMContentLoaded', function () {
        // Assign the previous
        var careerInterest = localStorage.TAMCareerInterest
        preparedResultBoard(careerInterest);     
    });
